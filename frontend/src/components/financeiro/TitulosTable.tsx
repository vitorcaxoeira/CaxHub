        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className="hidden bg-surface-2 px-5 py-3 text-right font-mono text-[10px] font-medium uppercase tracking-wider text-muted sm:table-cell">
                Dias Atraso
              </th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.codemp}-${row.codfil}-${row.numtit}-${row.codtpt}`} className="border-t border-border/60 transition hover:bg-surface-2">
                ...
                <td className="hidden px-5 py-3.5 text-right font-mono text-sm tabular-nums text-muted sm:table-cell">
                  {row.dias_atraso}
                </td>
                <td className="px-5 py-3.5 text-right font-mono text-sm font-semibold tabular-nums text-foreground">
                  R$ {currencyFormatter.format(row.vlrabe)}
                </td>
