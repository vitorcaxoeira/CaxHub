...
const statusTone: Record<string, string> = {
  ...
  enviando: "bg-warning/15 text-warning",
  ...
  // (que É uma falha real de envio, esgotou tentativas). Só existe na fila de Atividades —
  // despesa nunca cai em "invalido" (ver PainelDespesas).
...
// Tela "Exportados para o Senior" — hoje cobre duas filas IRMÃS, mas fisicamente separadas
// (tabelas diferentes, sem relação uma com a outra): SincronizacaoPendente (apontamento e
// alocação, ligada a AtividadeConsultor) e SincronizacaoPendenteDespesa (despesas de viagem,
// ligada a RegistroDespesaViagem — motivo de não caber na mesma tabela: ver o comentário do
// model no schema.prisma). Cada aba abaixo é um painel independente, com fetch/paginação
// próprios — juntar as duas listas numa paginação só exigiria misturar dois formatos de linha
// bem diferentes (Proposta/Atividade vs RAT/Despesa) só pra economizar uma aba a mais.
export function SincronizacaoSenior() {
  const [aba, setAba] = useState<"rat" | "despesa">("rat");
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted">
          ...
          (<code>registrarAtividades</code>), alocação (<code>alocarAtividades</code>) e despesa de viagem
          (<code>ManterItemDespesa</code>) já têm canal publicado; um item de Atividades só fica represado sem
          consumir tentativa quando o tipo ainda não tem operação do lado do Senior.
        </p>
      </div>

      <Tabs
        tabs={[
          { key: "rat", label: "Atividades" },
          { key: "despesa", label: "Despesas de viagem" },
        ]}
        activeKey={aba}
        onChange={(key) => setAba(key as "rat" | "despesa")}
      />

      {aba === "rat" ? <PainelAtividades /> : <PainelDespesas />}
    </div>
  );
}
...
function PainelAtividades() {
  ...
  return (
    <>
      {loadingIndicadores ? (
        <div className="mb-6 mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
...
// ---------- Aba "Despesas de viagem" ----------
//
// Espelho de PainelAtividades acima, lendo SincronizacaoPendenteDespesa em vez de
// SincronizacaoPendente (GET /sincronizacao/despesas*) — mesmo layout/ações (Ver payload,
// Enviar para o Senior), sem "Marcar como inválido": despesa só entra na fila depois de
// validada (validarCamposDespesa em routes/rats.ts), não existe o caso "enfileirada sem dado
// suficiente pra tentar" que justifica essa ação do lado de Atividades.
interface ItemSincronizacaoDespesa {
  id: number;
  despesaId: number;
  // Id interno de Rat (pra linkar RatVisualizacao) — null no caso raro de a RAT ter sumido
  // depois da despesa ter sido enfileirada.
  ratId: number | null;
  codemp: number;
  numrat: number;
  desrdv: string | null;
...
const TIPO_LABEL_DESPESA: Record<string, string> = {
  enviar_despesa: "Lançar despesa",
  editar_despesa: "Editar despesa",
  excluir_despesa: "Excluir despesa",
};

const TIPO_OPCOES_DESPESA: MultiSelectOption<string>[] = Object.entries(TIPO_LABEL_DESPESA).map(([value, label]) => ({
  value,
  label,
}));

const currencyDespesa = new Intl.NumberFormat("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const formatMoneyDespesa = (v: number | null) => (v == null ? "—" : `R$ ${currencyDespesa.format(v)}`);

function PainelDespesas() {
  const [itens, setItens] = useState<ItemSincronizacaoDespesa[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [erro, setErro] = useState<string | null>(null);
  const [reprocessando, setReprocessando] = useState<number | null>(null);
  const [itemDoPayload, setItemDoPayload] = useState<ItemSincronizacaoDespesa | null>(null);
  const [copiado, setCopiado] = useState(false);
  const [preview, setPreview] = useState<PreviewEnvio | null>(null);
  const [previewCarregando, setPreviewCarregando] = useState(false);
  const [previewErro, setPreviewErro] = useState<string | null>(null);
  const [copiadoPreview, setCopiadoPreview] = useState(false);
  const [itemDoErro, setItemDoErro] = useState<ItemSincronizacaoDespesa | null>(null);
  const [copiadoErro, setCopiadoErro] = useState(false);
  ...
  const [despesaIdInput, setDespesaIdInput] = useState("");
  const despesaIdDebounced = useDebouncedValue(despesaIdInput, 350);
  ...
  function carregar() {
    setLoading(true);
    axios
      .get("/api/sincronizacao/despesas", {
        params: {
          page,
          pageSize: PAGE_SIZE,
          status: statusFiltro || undefined,
          ...
          despesaId: despesaIdDebounced || undefined,
        },
      })
      .then(({ data }) => {
        setItens(data.itens);
  ...
  function carregarIndicadores() {
    setLoadingIndicadores(true);
    axios
      .get("/api/sincronizacao/despesas/indicadores")
      .then(({ data }) => setIndicadores(data))
      .catch(() => {})
      .finally(() => setLoadingIndicadores(false));
  }
  ...
  useEffect(() => {
    carregar();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, statusFiltro, tipoFiltro, numratDebounced, despesaIdDebounced]);

  useEffect(() => {
    setPage(1);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [statusFiltro, tipoFiltro, numratDebounced, despesaIdDebounced]);

  function alternarStatusFiltro(status: string) {
    setStatusFiltro((atual) => (atual === status ? null : status));
  }

  function abrirPayload(item: ItemSincronizacaoDespesa) {
    setItemDoPayload(item);
    setPreview(null);
    setPreviewErro(null);
    setPreviewCarregando(true);
    axios
      .get(`/api/sincronizacao/despesas/${item.id}/preview`)
      .then(({ data }) => setPreview(data))
      .catch((err) => setPreviewErro(err.response?.data?.error ?? "Falha ao reconstruir o envio"))
      .finally(() => setPreviewCarregando(false));
  }
  ...
  async function reprocessar(id: number) {
    setReprocessando(id);
    try {
      await axios.post(`/api/sincronizacao/despesas/${id}/reprocessar`);
      carregar();
      carregarIndicadores();
    } catch (err: any) {
      setErro(err.response?.data?.error ?? "Falha ao reprocessar item");
  ...
  return (
    <>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        />
        <input
          value={despesaIdInput}
          onChange={(e) => setDespesaIdInput(e.target.value)}
          placeholder="Id da despesa... (separe por vírgula)"
          className="w-56 rounded-md border border-border bg-surface px-3 py-1.5 text-sm text-foreground placeholder:text-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        />
        <MultiSelectDropdown
          opcoes={TIPO_OPCOES_DESPESA}
          selecionados={tipoFiltro}
          onChange={setTipoFiltro}
          labelTodos="Todos os tipos"
          labelSufixo="tipos"
              ...
              {!loading &&
                itens.map((item) => (
                  <tr key={item.id} className="border-t border-border/60 transition hover:bg-surface-2">
                    ...
                    <td className="whitespace-nowrap px-3 py-3.5 text-sm font-semibold">
                      {item.ratId != null ? (
                        <Link to={`/projetos/rat/${item.ratId}`} className="text-primary hover:underline">
                          {item.numrat}
                        </Link>
                      ) : (
                        item.numrat
                    ...
                    <td className="whitespace-nowrap px-3 py-3.5 text-sm text-muted">{TIPO_LABEL_DESPESA[item.tipo] ?? item.tipo}</td>
                    <td className="max-w-[240px] px-3 py-3.5 text-[12.5px]">
                      <p className="truncate text-foreground" title={item.desrdv ?? undefined}>
                        {item.desrdv ?? "—"}
                      </p>
                      <p className="mt-0.5 font-mono tabular-nums text-muted">{formatMoneyDespesa(item.vlrtot)}</p>
                    </td>
                    <td className="whitespace-nowrap px-3 py-3.5 text-right font-mono text-sm tabular-nums text-muted">{item.tentativas}</td>
                    <td className="max-w-[280px] px-3 py-3.5 text-[12px]">
                      {item.ultimoErro ? (
      ...
      <Modal
        ...
        title={`Payload — ${itemDoPayload ? TIPO_LABEL_DESPESA[itemDoPayload.tipo] ?? itemDoPayload.tipo : ""}`}
        subtitulo={itemDoPayload ? `RAT ${itemDoPayload.numrat} · pendência #${itemDoPayload.id}` : undefined}
      >
        {itemDoPayload && (
          <>
            <p className="mb-1.5 text-[11.5px] font-medium uppercase tracking-wide text-muted">
      ...
      <Modal
        ...
        title={`Erro — ${itemDoErro ? TIPO_LABEL_DESPESA[itemDoErro.tipo] ?? itemDoErro.tipo : ""}`}
        subtitulo={itemDoErro ? `RAT ${itemDoErro.numrat} · pendência #${itemDoErro.id}` : undefined}
      >
        {itemDoErro && (
          <>
            <p className="whitespace-pre-wrap rounded-md bg-surface-2 p-3 text-[12.5px] text-foreground">
