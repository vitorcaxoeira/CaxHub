...
export const BASE_QUERY =`SELECT codemp AS codemp, numcco AS numcco, datmov AS datmov, seqmov AS seqmov, codfil AS codfil, vlrmov AS vlrmov, debcre AS debcre, hismov AS hismov, sitmcc AS sitmcc, filmcr AS filmcr, nummcr AS nummcr, tptmcr AS tptmcr, seqmcr AS seqmcr, codpor AS codpor FROM e600mcc`;
...
// schema.prisma (MovimentoConta): vlrmov Decimal(15,2), debcre VarChar(1) NOT NULL.
...
export async function runMovimentoContaSync(desde?: Date): Promise<void> {
  ...
  try {
    ...
    const varredura = await executarVarreduraDoJob<Prisma.MovimentoContaWhereInput>(prisma.movimentoConta, {
      jobName: JOB_NAME,
