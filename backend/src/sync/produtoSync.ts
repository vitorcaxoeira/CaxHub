...
export async function runProdutoSync(desde?: Date): Promise<void> {
  ...
  try {
    ...
    // `{}`: Produto é espelho só-leitura (Senior -> CaxHub), sem escrita de volta, então não
    // há registro "nascido no CaxHub" pra excluir do escopo. `executarVarreduraDoJob` decide
    // sozinho se pode rodar (incremental nunca varre; filtro não-escopável nunca varre).
    // Começa em "desligada" (default do model ConfiguracaoVarredura) — promoção pela tela é
    // decisão manual, não desta leva.
    const varredura = await executarVarreduraDoJob<Prisma.ProdutoWhereInput>(prisma.produto, {
      jobName: JOB_NAME,
