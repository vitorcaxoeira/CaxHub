...
export const BASE_QUERY =`SELECT USU_DepExe AS depexe, USU_CodEmp AS codemp, USU_CodUsu AS codusu, USU_UsuGer AS usuger, USU_DatGer AS datger, USU_HorGer AS horger, USU_SitReg AS sitreg FROM USU_TDepExeTim`;
...
export async function runDepartamentoTimeSync(desde?: Date): Promise<void> {
  ...
  try {
    ...
    const varredura = await executarVarreduraDoJob<Prisma.DepartamentoTimeWhereInput>(prisma.departamentoTime, {
      jobName: JOB_NAME,
