...
// `contarRemovidos`/`listarRemovidos` genéricos (10/09/2026, porte do CaxHub_Atlas) — antes só
// Pedido/Lançamento Contábil/Rateio de Lançamento tinham essas duas funções, escritas à mão.
// Em vez de repetir o mesmo par ~41 vezes pras tabelas que ganharam a varredura nesta leva,
// reaproveita o truque de `Prisma.dmmf` que `recorteRetroativo.ts` já usa pra achar o delegate
// certo a partir de `tabelaLocal` em runtime. Os 3 jobs acima continuam com a versão própria
// (rótulo mais rico: nome do cliente/situação/valor) — não precisa trocar algo que já funciona
// por uma versão genérica pior. Seguro até pras 4 tabelas de mão dupla (AtividadeConsultor,
// Rat, RatItem, RegistroDespesaViagem): o escopo especial delas entra na hora de MARCAR (ver
// varrerRemovidos.ts), não na hora de LISTAR o que já foi marcado — listar é sempre "WHERE
// removidoEmSenior IS NOT NULL", igual em qualquer tabela.
function contarRemovidosGenerico(tabelaLocal: string): () => Promise<number> {
  return () => delegatePorTabelaLocal(tabelaLocal).count({ where: { removidoEmSenior: { not: null } } });
}

function listarRemovidosGenerico(
  tabelaLocal: string,
  displayName: string
): (limite: number, candidatosDesde: Date | null) => Promise<ItemRemovido[]> {
  return async (limite, candidatosDesde) => {
    const delegate = delegatePorTabelaLocal(tabelaLocal);
    const pk = pkFieldsDoModel(tabelaLocal);
    const linhas = await delegate.findMany({
      where: candidatosDesde
        ? // Marcados + os que a varredura marcaria agora (útil em modo "simular", onde os
          // primeiros ainda não existem). `lt` estrito ignora carimbo NULL, então registro
          // nascido fora do sync (mão dupla) é imune por construção.
          { OR: [{ removidoEmSenior: { not: null } }, { vistoEmSync: { lt: candidatosDesde } }] }
        : { removidoEmSenior: { not: null } },
      orderBy: { removidoEmSenior: "desc" },
      take: limite,
    });
    return linhas.map((linha) => {
      const chave = pk.map((campo) => String(linha[campo])).join("/");
      return {
        chave,
        rotulo: `${displayName} ${chave}`,
        removidoEmSenior: linha.removidoEmSenior as Date | null,
        marcado: linha.removidoEmSenior != null,
      };
    });
  };
}
...
export const SYNC_JOBS: SyncJobDescriptor[] = [
  { jobName: EMPRESA_JOB, displayName: "Empresas", cronExpr: EMPRESA_CRON, suportaAlterados: EMPRESA_DATA != null, campoData: EMPRESA_DATA, ...catalogo(EMPRESA_JOB, EMPRESA_QUERY, "empresa"), run: runEmpresaSync, contarRegistros: () => prisma.empresa.count(), contarRemovidos: contarRemovidosGenerico("empresa"), listarRemovidos: listarRemovidosGenerico("empresa", "Empresa") },
  { jobName: FILIAL_JOB, displayName: "Filiais", cronExpr: FILIAL_CRON, suportaAlterados: FILIAL_DATA != null, campoData: FILIAL_DATA, ...catalogo(FILIAL_JOB, FILIAL_QUERY, "filial"), run: runFilialSync, contarRegistros: () => prisma.filial.count(), contarRemovidos: contarRemovidosGenerico("filial"), listarRemovidos: listarRemovidosGenerico("filial", "Filial") },
  { jobName: CLIENTE_JOB, displayName: "Clientes", cronExpr: CLIENTE_CRON, suportaAlterados: CLIENTE_DATA != null, campoData: CLIENTE_DATA, ...catalogo(CLIENTE_JOB, CLIENTE_QUERY, "clientes"), run: runClienteSync, contarRegistros: () => prisma.cliente.count(), contarRemovidos: contarRemovidosGenerico("clientes"), listarRemovidos: listarRemovidosGenerico("clientes", "Cliente") },
  { jobName: TIPO_TITULO_JOB, displayName: "Tipos de Título", cronExpr: TIPO_TITULO_CRON, suportaAlterados: TIPO_TITULO_DATA != null, campoData: TIPO_TITULO_DATA, ...catalogo(TIPO_TITULO_JOB, TIPO_TITULO_QUERY, "tipos_titulo"), run: runTipoTituloSync, contarRegistros: () => prisma.tipoTitulo.count(), contarRemovidos: contarRemovidosGenerico("tipos_titulo"), listarRemovidos: listarRemovidosGenerico("tipos_titulo", "Tipo de Título") },
  { jobName: PORTADOR_JOB, displayName: "Portadores", cronExpr: PORTADOR_CRON, suportaAlterados: PORTADOR_DATA != null, campoData: PORTADOR_DATA, ...catalogo(PORTADOR_JOB, PORTADOR_QUERY, "portadores"), run: runPortadorSync, contarRegistros: () => prisma.portador.count(), contarRemovidos: contarRemovidosGenerico("portadores"), listarRemovidos: listarRemovidosGenerico("portadores", "Portador") },
  { jobName: TRANSACAO_JOB, displayName: "Transações", cronExpr: TRANSACAO_CRON, suportaAlterados: TRANSACAO_DATA != null, campoData: TRANSACAO_DATA, ...catalogo(TRANSACAO_JOB, TRANSACAO_QUERY, "transacoes"), run: runTransacaoSync, contarRegistros: () => prisma.transacao.count(), contarRemovidos: contarRemovidosGenerico("transacoes"), listarRemovidos: listarRemovidosGenerico("transacoes", "Transação") },
  { jobName: TITULO_RECEBER_JOB, displayName: "Títulos a Receber", cronExpr: TITULO_RECEBER_CRON, suportaAlterados: TITULO_RECEBER_DATA != null, campoData: TITULO_RECEBER_DATA, ...catalogo(TITULO_RECEBER_JOB, TITULO_RECEBER_QUERY, "titulos_receber"), run: runTituloReceberSync, contarRegistros: () => prisma.tituloReceber.count(), contarRemovidos: contarRemovidosGenerico("titulos_receber"), listarRemovidos: listarRemovidosGenerico("titulos_receber", "Título a Receber") },
  { jobName: MOVIMENTO_TITULO_JOB, displayName: "Movimentos de Títulos a Receber", cronExpr: MOVIMENTO_TITULO_CRON, suportaAlterados: MOVIMENTO_TITULO_DATA != null, campoData: MOVIMENTO_TITULO_DATA, ...catalogo(MOVIMENTO_TITULO_JOB, MOVIMENTO_TITULO_QUERY, "movimentos_receber"), run: runMovimentoTituloReceberSync, contarRegistros: () => prisma.movimentoTituloReceber.count(), contarRemovidos: contarRemovidosGenerico("movimentos_receber"), listarRemovidos: listarRemovidosGenerico("movimentos_receber", "Movimento de Título") },
  { jobName: REPRESENTANTE_JOB, displayName: "Representantes", cronExpr: REPRESENTANTE_CRON, suportaAlterados: REPRESENTANTE_DATA != null, campoData: REPRESENTANTE_DATA, ...catalogo(REPRESENTANTE_JOB, REPRESENTANTE_QUERY, "representantes"), run: runRepresentanteSync, contarRegistros: () => prisma.representante.count(), contarRemovidos: contarRemovidosGenerico("representantes"), listarRemovidos: listarRemovidosGenerico("representantes", "Representante") },
  { jobName: CENTRO_CUSTO_JOB, displayName: "Centros de Custo", cronExpr: CENTRO_CUSTO_CRON, suportaAlterados: CENTRO_CUSTO_DATA != null, campoData: CENTRO_CUSTO_DATA, ...catalogo(CENTRO_CUSTO_JOB, CENTRO_CUSTO_QUERY, "centros_custo"), run: runCentroCustoSync, contarRegistros: () => prisma.centroCusto.count(), contarRemovidos: contarRemovidosGenerico("centros_custo"), listarRemovidos: listarRemovidosGenerico("centros_custo", "Centro de Custo") },
  { jobName: MOVIMENTO_CONTA_JOB, displayName: "Movimentos de Conta", cronExpr: MOVIMENTO_CONTA_CRON, suportaAlterados: MOVIMENTO_CONTA_DATA != null, campoData: MOVIMENTO_CONTA_DATA, ...catalogo(MOVIMENTO_CONTA_JOB, MOVIMENTO_CONTA_QUERY, "movimentos_conta"), run: runMovimentoContaSync, contarRegistros: () => prisma.movimentoConta.count(), contarRemovidos: contarRemovidosGenerico("movimentos_conta"), listarRemovidos: listarRemovidosGenerico("movimentos_conta", "Movimento de Conta") },
  { jobName: NATUREZA_FINANCEIRA_JOB, displayName: "Naturezas Financeiras", cronExpr: NATUREZA_FINANCEIRA_CRON, suportaAlterados: NATUREZA_FINANCEIRA_DATA != null, campoData: NATUREZA_FINANCEIRA_DATA, ...catalogo(NATUREZA_FINANCEIRA_JOB, NATUREZA_FINANCEIRA_QUERY, "naturezas_financeiras"), run: runNaturezaFinanceiraSync, contarRegistros: () => prisma.naturezaFinanceira.count(), contarRemovidos: contarRemovidosGenerico("naturezas_financeiras"), listarRemovidos: listarRemovidosGenerico("naturezas_financeiras", "Natureza Financeira") },
  { jobName: MOEDA_JOB, displayName: "Moedas", cronExpr: MOEDA_CRON, suportaAlterados: MOEDA_DATA != null, campoData: MOEDA_DATA, ...catalogo(MOEDA_JOB, MOEDA_QUERY, "moedas"), run: runMoedaSync, contarRegistros: () => prisma.moeda.count(), contarRemovidos: contarRemovidosGenerico("moedas"), listarRemovidos: listarRemovidosGenerico("moedas", "Moeda") },
  { jobName: CONTA_CORRENTE_JOB, displayName: "Contas Correntes", cronExpr: CONTA_CORRENTE_CRON, suportaAlterados: CONTA_CORRENTE_DATA != null, campoData: CONTA_CORRENTE_DATA, ...catalogo(CONTA_CORRENTE_JOB, CONTA_CORRENTE_QUERY, "contas_correntes"), run: runContaCorrenteSync, contarRegistros: () => prisma.contaCorrente.count(), contarRemovidos: contarRemovidosGenerico("contas_correntes"), listarRemovidos: listarRemovidosGenerico("contas_correntes", "Conta Corrente") },
  { jobName: PROPOSTA_JOB, displayName: "Propostas", cronExpr: PROPOSTA_CRON, suportaAlterados: PROPOSTA_DATA != null, campoData: PROPOSTA_DATA, ...catalogo(PROPOSTA_JOB, PROPOSTA_QUERY, "propostas"), run: runPropostaSync, contarRegistros: () => prisma.proposta.count(), contarRemovidos: contarRemovidosGenerico("propostas"), listarRemovidos: listarRemovidosGenerico("propostas", "Proposta") },
  { jobName: PROPOSTA_ITEM_JOB, displayName: "Itens de Proposta", cronExpr: PROPOSTA_ITEM_CRON, suportaAlterados: PROPOSTA_ITEM_DATA != null, campoData: PROPOSTA_ITEM_DATA, ...catalogo(PROPOSTA_ITEM_JOB, PROPOSTA_ITEM_QUERY, "propostas_itens"), run: runPropostaItemSync, contarRegistros: () => prisma.propostaItem.count(), contarRemovidos: contarRemovidosGenerico("propostas_itens"), listarRemovidos: listarRemovidosGenerico("propostas_itens", "Item de Proposta") },
  { jobName: CONSULTOR_JOB, displayName: "Consultores", cronExpr: CONSULTOR_CRON, suportaAlterados: CONSULTOR_DATA != null, campoData: CONSULTOR_DATA, ...catalogo(CONSULTOR_JOB, CONSULTOR_QUERY, "consultores"), run: runConsultorSync, contarRegistros: () => prisma.consultor.count(), contarRemovidos: contarRemovidosGenerico("consultores"), listarRemovidos: listarRemovidosGenerico("consultores", "Consultor") },
  ...
  { jobName: CONTRATO_CONSULTOR_JOB, displayName: "Contratos de Consultores (Valor-hora)", cronExpr: CONTRATO_CONSULTOR_CRON, suportaAlterados: CONTRATO_CONSULTOR_DATA != null, campoData: CONTRATO_CONSULTOR_DATA, ...catalogo(CONTRATO_CONSULTOR_JOB, CONTRATO_CONSULTOR_QUERY, "contratos_consultores"), run: runContratoConsultorSync, contarRegistros: () => prisma.contratoConsultor.count(), contarRemovidos: contarRemovidosGenerico("contratos_consultores"), listarRemovidos: listarRemovidosGenerico("contratos_consultores", "Contrato de Consultor") },
  { jobName: DEPARTAMENTO_GESTOR_JOB, displayName: "Gestores de Departamento", cronExpr: DEPARTAMENTO_GESTOR_CRON, suportaAlterados: DEPARTAMENTO_GESTOR_DATA != null, campoData: DEPARTAMENTO_GESTOR_DATA, ...catalogo(DEPARTAMENTO_GESTOR_JOB, DEPARTAMENTO_GESTOR_QUERY, "departamentos_gestores"), run: runDepartamentoGestorSync, contarRegistros: () => prisma.departamentoGestor.count(), contarRemovidos: contarRemovidosGenerico("departamentos_gestores"), listarRemovidos: listarRemovidosGenerico("departamentos_gestores", "Gestor de Departamento") },
  { jobName: DEPARTAMENTO_TIME_JOB, displayName: "Time por Departamento", cronExpr: DEPARTAMENTO_TIME_CRON, suportaAlterados: DEPARTAMENTO_TIME_DATA != null, campoData: DEPARTAMENTO_TIME_DATA, ...catalogo(DEPARTAMENTO_TIME_JOB, DEPARTAMENTO_TIME_QUERY, "departamento_time"), run: runDepartamentoTimeSync, contarRegistros: () => prisma.departamentoTime.count(), contarRemovidos: contarRemovidosGenerico("departamento_time"), listarRemovidos: listarRemovidosGenerico("departamento_time", "Time de Departamento") },
  // FaseProposta roda antes de AtividadeConsultor: AtividadeConsultor.fasid é FK pra fases_proposta.
  { jobName: FASE_PROPOSTA_JOB, displayName: "Fases de Proposta", cronExpr: FASE_PROPOSTA_CRON, suportaAlterados: FASE_PROPOSTA_DATA != null, campoData: FASE_PROPOSTA_DATA, ...catalogo(FASE_PROPOSTA_JOB, FASE_PROPOSTA_QUERY, "fases_proposta"), run: runFasePropostaSync, contarRegistros: () => prisma.faseProposta.count(), contarRemovidos: contarRemovidosGenerico("fases_proposta"), listarRemovidos: listarRemovidosGenerico("fases_proposta", "Fase de Proposta") },
  { jobName: ATIVIDADE_CONSULTOR_JOB, displayName: "Atividades por Consultor", cronExpr: ATIVIDADE_CONSULTOR_CRON, suportaAlterados: ATIVIDADE_CONSULTOR_DATA != null, campoData: ATIVIDADE_CONSULTOR_DATA, ...catalogo(ATIVIDADE_CONSULTOR_JOB, ATIVIDADE_CONSULTOR_QUERY, "atividades_consultor"), run: runAtividadeConsultorSync, contarRegistros: () => prisma.atividadeConsultor.count(), contarRemovidos: contarRemovidosGenerico("atividades_consultor"), listarRemovidos: listarRemovidosGenerico("atividades_consultor", "Atividade") },
  { jobName: RAT_JOB, displayName: "RATs (Cabeçalho)", cronExpr: RAT_CRON, suportaAlterados: RAT_DATA != null, campoData: RAT_DATA, ...catalogo(RAT_JOB, RAT_QUERY, "rats"), run: runRatSync, contarRegistros: () => prisma.rat.count(), contarRemovidos: contarRemovidosGenerico("rats"), listarRemovidos: listarRemovidosGenerico("rats", "RAT") },
  // RatItem roda depois de Rat: RatItem.ratId é resolvido casando (codemp, numrat, codpro) contra Rat já sincronizado.
  { jobName: RAT_ITEM_JOB, displayName: "Itens de RAT (Apontamentos)", cronExpr: RAT_ITEM_CRON, suportaAlterados: RAT_ITEM_DATA != null, campoData: RAT_ITEM_DATA, ...catalogo(RAT_ITEM_JOB, RAT_ITEM_QUERY, "rat_itens"), run: runRatItemSync, contarRegistros: () => prisma.ratItem.count(), contarRemovidos: contarRemovidosGenerico("rat_itens"), listarRemovidos: listarRemovidosGenerico("rat_itens", "Item de RAT") },
  ...
  { jobName: FORMA_PAGAMENTO_JOB, displayName: "Formas de Pagamento", cronExpr: FORMA_PAGAMENTO_CRON, suportaAlterados: FORMA_PAGAMENTO_DATA != null, campoData: FORMA_PAGAMENTO_DATA, ...catalogo(FORMA_PAGAMENTO_JOB, FORMA_PAGAMENTO_QUERY, "formas_pagamento"), run: runFormaPagamentoSync, contarRegistros: () => prisma.formaPagamento.count(), contarRemovidos: contarRemovidosGenerico("formas_pagamento"), listarRemovidos: listarRemovidosGenerico("formas_pagamento", "Forma de Pagamento") },
  { jobName: CONDICAO_PAGAMENTO_JOB, displayName: "Condições de Pagamento", cronExpr: CONDICAO_PAGAMENTO_CRON, suportaAlterados: CONDICAO_PAGAMENTO_DATA != null, campoData: CONDICAO_PAGAMENTO_DATA, ...catalogo(CONDICAO_PAGAMENTO_JOB, CONDICAO_PAGAMENTO_QUERY, "condicoes_pagamento"), run: runCondicaoPagamentoSync, contarRegistros: () => prisma.condicaoPagamento.count(), contarRemovidos: contarRemovidosGenerico("condicoes_pagamento"), listarRemovidos: listarRemovidosGenerico("condicoes_pagamento", "Condição de Pagamento") },
  ...
  { jobName: PLANO_CONTABIL_JOB, displayName: "Plano Contábil", cronExpr: PLANO_CONTABIL_CRON, suportaAlterados: PLANO_CONTABIL_DATA != null, campoData: PLANO_CONTABIL_DATA, ...catalogo(PLANO_CONTABIL_JOB, PLANO_CONTABIL_QUERY, "plano_contabil"), run: runPlanoContabilSync, contarRegistros: () => prisma.planoContabil.count(), contarRemovidos: contarRemovidosGenerico("plano_contabil"), listarRemovidos: listarRemovidosGenerico("plano_contabil", "Conta do Plano Contábil") },
  ...
  { jobName: ORCAMENTO_CONTABIL_JOB, displayName: "Orçamentos Contábeis", cronExpr: ORCAMENTO_CONTABIL_CRON, suportaAlterados: ORCAMENTO_CONTABIL_DATA != null, campoData: ORCAMENTO_CONTABIL_DATA, ...catalogo(ORCAMENTO_CONTABIL_JOB, ORCAMENTO_CONTABIL_QUERY, "orcamentos_contabeis"), run: runOrcamentoContabilSync, contarRegistros: () => prisma.orcamentoContabil.count(), contarRemovidos: contarRemovidosGenerico("orcamentos_contabeis"), listarRemovidos: listarRemovidosGenerico("orcamentos_contabeis", "Orçamento Contábil") },
  ...
  // — sem dependência das tabelas acima. Até 10/09/2026 ficava sem varredura de propósito
  // (catálogo global, raramente muda) — revertido: ver comentário em schema.prisma.
  { jobName: HISTORICO_PADRAO_JOB, displayName: "Históricos Padrão", cronExpr: HISTORICO_PADRAO_CRON, suportaAlterados: HISTORICO_PADRAO_DATA != null, campoData: HISTORICO_PADRAO_DATA, ...catalogo(HISTORICO_PADRAO_JOB, HISTORICO_PADRAO_QUERY, "historicos_padrao"), run: runHistoricoPadraoSync, contarRegistros: () => prisma.historicoPadrao.count(), contarRemovidos: contarRemovidosGenerico("historicos_padrao"), listarRemovidos: listarRemovidosGenerico("historicos_padrao", "Histórico Padrão") },
  ...
  { jobName: ROTA_VIAGEM_JOB, displayName: "Rotas de Viagem", cronExpr: ROTA_VIAGEM_CRON, suportaAlterados: ROTA_VIAGEM_DATA != null, campoData: ROTA_VIAGEM_DATA, ...catalogo(ROTA_VIAGEM_JOB, ROTA_VIAGEM_QUERY, "rotas_viagem"), run: runRotaViagemSync, contarRegistros: () => prisma.rotaViagem.count(), contarRemovidos: contarRemovidosGenerico("rotas_viagem"), listarRemovidos: listarRemovidosGenerico("rotas_viagem", "Rota de Viagem") },
  { jobName: PERCURSO_VIAGEM_JOB, displayName: "Percursos de Viagem", cronExpr: PERCURSO_VIAGEM_CRON, suportaAlterados: PERCURSO_VIAGEM_DATA != null, campoData: PERCURSO_VIAGEM_DATA, ...catalogo(PERCURSO_VIAGEM_JOB, PERCURSO_VIAGEM_QUERY, "percursos_viagem"), run: runPercursoViagemSync, contarRegistros: () => prisma.percursoViagem.count(), contarRemovidos: contarRemovidosGenerico("percursos_viagem"), listarRemovidos: listarRemovidosGenerico("percursos_viagem", "Percurso de Viagem") },
  { jobName: ROTA_PERCURSO_JOB, displayName: "Rotas x Percursos", cronExpr: ROTA_PERCURSO_CRON, suportaAlterados: ROTA_PERCURSO_DATA != null, campoData: ROTA_PERCURSO_DATA, ...catalogo(ROTA_PERCURSO_JOB, ROTA_PERCURSO_QUERY, "rotas_percursos"), run: runRotaPercursoSync, contarRegistros: () => prisma.rotaPercurso.count(), contarRemovidos: contarRemovidosGenerico("rotas_percursos"), listarRemovidos: listarRemovidosGenerico("rotas_percursos", "Rota x Percurso") },
  { jobName: REGISTRO_DESPESA_VIAGEM_JOB, displayName: "Despesas de Viagem (RAT)", cronExpr: REGISTRO_DESPESA_VIAGEM_CRON, suportaAlterados: REGISTRO_DESPESA_VIAGEM_DATA != null, campoData: REGISTRO_DESPESA_VIAGEM_DATA, ...catalogo(REGISTRO_DESPESA_VIAGEM_JOB, REGISTRO_DESPESA_VIAGEM_QUERY, "registros_despesa_viagem"), run: runRegistroDespesaViagemSync, contarRegistros: () => prisma.registroDespesaViagem.count(), contarRemovidos: contarRemovidosGenerico("registros_despesa_viagem"), listarRemovidos: listarRemovidosGenerico("registros_despesa_viagem", "Despesa de Viagem") },
  ...
  { jobName: PRODUTO_JOB, displayName: "Produtos", cronExpr: PRODUTO_CRON, suportaAlterados: PRODUTO_DATA != null, campoData: PRODUTO_DATA, ...catalogo(PRODUTO_JOB, PRODUTO_QUERY, "produtos"), run: runProdutoSync, contarRegistros: () => prisma.produto.count(), contarRemovidos: contarRemovidosGenerico("produtos"), listarRemovidos: listarRemovidosGenerico("produtos", "Produto") },
  { jobName: DERIVACAO_PRODUTO_JOB, displayName: "Derivações de Produto", cronExpr: DERIVACAO_PRODUTO_CRON, suportaAlterados: DERIVACAO_PRODUTO_DATA != null, campoData: DERIVACAO_PRODUTO_DATA, ...catalogo(DERIVACAO_PRODUTO_JOB, DERIVACAO_PRODUTO_QUERY, "derivacoes_produto"), run: runDerivacaoProdutoSync, contarRegistros: () => prisma.derivacaoProduto.count(), contarRemovidos: contarRemovidosGenerico("derivacoes_produto"), listarRemovidos: listarRemovidosGenerico("derivacoes_produto", "Derivação de Produto") },
  { jobName: SERVICO_JOB, displayName: "Serviços", cronExpr: SERVICO_CRON, suportaAlterados: SERVICO_DATA != null, campoData: SERVICO_DATA, ...catalogo(SERVICO_JOB, SERVICO_QUERY, "servicos"), run: runServicoSync, contarRegistros: () => prisma.servico.count(), contarRemovidos: contarRemovidosGenerico("servicos"), listarRemovidos: listarRemovidosGenerico("servicos", "Serviço") },
  ...
  { jobName: NOTA_FISCAL_VENDA_JOB, displayName: "Notas Fiscais de Venda", cronExpr: NOTA_FISCAL_VENDA_CRON, suportaAlterados: NOTA_FISCAL_VENDA_DATA != null, campoData: NOTA_FISCAL_VENDA_DATA, ...catalogo(NOTA_FISCAL_VENDA_JOB, NOTA_FISCAL_VENDA_QUERY, "notas_fiscais_venda"), run: runNotaFiscalVendaSync, contarRegistros: () => prisma.notaFiscalVenda.count(), contarRemovidos: contarRemovidosGenerico("notas_fiscais_venda"), listarRemovidos: listarRemovidosGenerico("notas_fiscais_venda", "Nota Fiscal de Venda") },
  { jobName: ITEM_SERVICO_NFV_JOB, displayName: "Itens de Serviço da NF", cronExpr: ITEM_SERVICO_NFV_CRON, suportaAlterados: ITEM_SERVICO_NFV_DATA != null, campoData: ITEM_SERVICO_NFV_DATA, ...catalogo(ITEM_SERVICO_NFV_JOB, ITEM_SERVICO_NFV_QUERY, "itens_servico_nfv"), run: runItemServicoNfVendaSync, contarRegistros: () => prisma.itemServicoNfVenda.count(), contarRemovidos: contarRemovidosGenerico("itens_servico_nfv"), listarRemovidos: listarRemovidosGenerico("itens_servico_nfv", "Item de Serviço da NF") },
  { jobName: ITEM_PRODUTO_NFV_JOB, displayName: "Itens de Produto da NF", cronExpr: ITEM_PRODUTO_NFV_CRON, suportaAlterados: ITEM_PRODUTO_NFV_DATA != null, campoData: ITEM_PRODUTO_NFV_DATA, ...catalogo(ITEM_PRODUTO_NFV_JOB, ITEM_PRODUTO_NFV_QUERY, "itens_produto_nfv"), run: runItemProdutoNfVendaSync, contarRegistros: () => prisma.itemProdutoNfVenda.count(), contarRemovidos: contarRemovidosGenerico("itens_produto_nfv"), listarRemovidos: listarRemovidosGenerico("itens_produto_nfv", "Item de Produto da NF") },
  { jobName: RATEIO_NFV_JOB, displayName: "Rateios de NF", cronExpr: RATEIO_NFV_CRON, suportaAlterados: RATEIO_NFV_DATA != null, campoData: RATEIO_NFV_DATA, ...catalogo(RATEIO_NFV_JOB, RATEIO_NFV_QUERY, "rateios_nfv"), run: runRateioNfVendaSync, contarRegistros: () => prisma.rateioNfVenda.count(), contarRemovidos: contarRemovidosGenerico("rateios_nfv"), listarRemovidos: listarRemovidosGenerico("rateios_nfv", "Rateio de NF") },
  ...
  { jobName: META_ANUAL_JOB, displayName: "Metas Anuais", cronExpr: META_ANUAL_CRON, suportaAlterados: META_ANUAL_DATA != null, campoData: META_ANUAL_DATA, ...catalogo(META_ANUAL_JOB, META_ANUAL_QUERY, "metas_anuais"), run: runMetaAnualSync, contarRegistros: () => prisma.metaAnual.count(), contarRemovidos: contarRemovidosGenerico("metas_anuais"), listarRemovidos: listarRemovidosGenerico("metas_anuais", "Meta Anual") },
];
